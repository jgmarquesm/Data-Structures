...
Node *_get_node(const LinkedList *LL, const unsigned int index) {
    Node *node = LL->begin;
...
void *LinkedList_remove_at(LinkedList *LL, const unsigned int index) {
    if (LL == NULL) {
...
void *LinkedList_get(const LinkedList *LL, const unsigned int index) {
    if (LL == NULL) {
        ...
        fprintf(stderr, "\nERROR: on function 'LinkedList_get'.\n");
        fprintf(stderr, "ERROR MESSAGE: Invalid index: %d. Try an index within [0, %lu].\n", index, LL->size - 1);
        exit(EXIT_FAILURE);
    } else {
...
void LinkedList_insert_at(LinkedList *LL, void *data, const unsigned int index) {
    if (LL == NULL) {
